...
Проект поддерживает сбор метрик и логов для мониторинга работы приложения.
...
- **`http_response_time_seconds`** - последнее время отклика

Для настройки полной системы мониторинга (Prometheus, Grafana, Loki) см. [MONITORING.md](MONITORING.md)

### Логирование

Все операции логируются в файл `app.log`:
- Загрузка контактов
- Добавление/обновление/удаление контактов
- Ошибки при работе с БД

Логи можно собирать с помощью Promtail и отправлять в Loki для анализа в Grafana.
